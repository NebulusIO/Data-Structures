...
int MyString::IndexOf(const char* str, int startIndex = 0) const{
  ...
  if(this->m_str[startIndex] == '\0')
    return;

  IndexOf(str, startIndex+1);

  char* indexFoundPtr = strstr(this->m_str, str);

  for(;startIndex < Length(this->m_str); startIndex++){

  }

  if(indexFoundPtr == nullptr)//str not found
    indexFound = -1;
  else
    indexFound =
